...
class ImageData(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.gray_transform is not None and self.need_sism:
            ...
            # print("before", sism.max(), sism.min())
            # sism = (sism-sism.min())/(sism.max()-sism.min())
            # print("after", sism.max(), sism.min())
        data_item = {}
        data_item['img'] = img
...
def get_loader(roots, request, batch_size, data_aug, shuffle, num_thread=4, pin=True, fix_seed=False):
    ...
    dataset = ImageData(roots, request, aug_transform=aug_transform, rgb_transform=rgb_transform,
                        gray_transform=gray_transform)
    cosal_sampler = Cosal_Sampler(indices=dataset.indices, shuffle=shuffle, batch_size=batch_size)
    if fix_seed:
        data_loader = data.DataLoader(dataset=dataset, batch_sampler=cosal_sampler, num_workers=num_thread,
                                      pin_memory=pin, worker_init_fn=_init_fn)
    else:
        data_loader = data.DataLoader(dataset=dataset, batch_sampler=cosal_sampler, num_workers=num_thread,
                                      pin_memory=pin)
    return data_loader
