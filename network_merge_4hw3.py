...
class Cosal_Sub_Module(nn.Module):
    ...
    def forward(self, feats, SISMs):
        ...
        def CFM(SIVs, NFs):
            ...
            CSA_maps = torch.sum(correlation_maps , dim=1)  # shape=[N, HW]
        ...
        def get_SCFs(NFs, pro):
            #NFs = NFs.view(N, C, HW)  # shape=[N, C, HW]
            #SCFs = torch.matmul(NFs.permute(0, 2, 1), NFs).view(N, -1, H, W)  # shape=[N, HW, H, W]
            SCFs = F.conv2d(NFs, weight=pro)

            return SCFs

        # Compute SIVs [Section 3.2, Equation 1].
        SIVs = F.normalize((NFs * SISMs).mean(dim=3).mean(dim=2), dim=1).view(N, C, 1, 1)  # shape=[N, C, 1, 1]
        mainpro = F.normalize((NFs * SISMs).mean(dim=3).mean(dim=2).mean(dim=0), dim=0)

        mainpro = mainpro.view(1, C)

        all_pro = NFs.reshape(N, C, HW).permute(0, 2, 1).reshape(N*HW, C)
        ind = torch.matmul(all_pro, mainpro.permute(1, 0))
        ind = torch.argsort(ind, dim=0, descending=True).repeat(1, C)
        main_pro = torch.gather(all_pro, dim=0, index=ind)[:HW, :].view(HW, C, 1, 1)
        ...
        SCFs = get_SCFs(NFs, main_pro)  # shape=[N, HW, H, W]

        # Rearrange the channel order of SCFs to obtain RSCFs [Section 3.4].
        #evidence = CSA_maps.view(N, HW)  # shape=[N, HW]
        #indices = torch.argsort(evidence, dim=1, descending=True).view(N, HW, 1, 1).repeat(1, 1, H,
        #                                                                                   W)  # shape=[N, HW, H, W]
        #RSCFs = torch.gather(SCFs, dim=1, index=indices)  # shape=[N, HW, H, W]
        cosal_feat = self.conv(SCFs * CSA_maps)  # shape=[N, 128, H, W]
        return cosal_feat
...
class ICNet(nn.Module):
    def __init__(self):
        ...
        self.conv3_cmprs = nn.Conv2d(256, 256, 1)
    ...
    def forward(self, image_group, SISMs, is_training):
        ...
        conv3_cmprs = self.conv3_cmprs(conv3_3)  # shape=[N, 128, 56, 56]
        ...
        cosal_feat_3 = self.Co3(conv3_cmprs, SISMs)  # shape=[N, 128, 28, 28]
        ...
        if is_training:
            preds_list = [resize(cosal_map_4),  resize(cosal_map_2), cosal_map_1]
            return preds_list
        else:
            preds = cosal_map_1
            return preds
