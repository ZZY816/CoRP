...
from loss import IoU_loss, BCE_loss
import numpy as np
import cv2
...
from data_loader_for_sal import Data
from data_loader_for_sal import Config
import network
...
class Solver(object):
    ...
    def train(self, roots, init_epoch, end_epoch, learning_rate, batch_size, weight_decay, ckpt_root, doc_path,
              num_thread, pin,  milestones, sal_root):
        ...
        backbone_lr = learning_rate / 1        # Reducing the backbone learning rate can make the training more stable.
        ...
        if init_epoch != 0:
            ...
            self.CoRP.Co6.apply(self.weights_init)
            self.CoRP.Co5.apply(self.weights_init)
            self.CoRP.Co4.apply(self.weights_init)
            self.CoRP.Co3.apply(self.weights_init)

            self.CoRP.conv6_cmprs.apply(self.weights_init)
            self.CoRP.conv5_cmprs.apply(self.weights_init)
            self.CoRP.conv4_cmprs.apply(self.weights_init)
            self.CoRP.conv3_cmprs.apply(self.weights_init)

            self.CoRP.merge_co_56.apply(self.weights_init)
            self.CoRP.merge_co_45.apply(self.weights_init)
            self.CoRP.merge_co_34.apply(self.weights_init)

            self.CoRP.get_pred_4.apply(self.weights_init)
            self.CoRP.refine_2.apply(self.weights_init)
            self.CoRP.refine_1.apply(self.weights_init)

            self.CoRP.sal_decoder.apply(self.weights_init)
        ...
        # train_sal_dataloader = DataLoader(All_data, collate_fn=All_data.collate, batch_size=8, shuffle=True, num_workers=8, worker_init_fn=_init_fn)
