...
def parse_args():
    ...
    parser.add_argument('--num', type=int, default=60,
                        help='The number of the epoch you want to evaluate.')

    parser.add_argument('--pred_root', type=str, default='./Predictions/pred_new/',
...
def main():
    args = parse_args()
    num = str(args.num)
    eval_doc_path = args.pred_root + 'evaluation_'+num+'.txt' # Txt Path to save the evaluation results.

    # An example to build "eval_roots".
    eval_roots = dict()
    for dataset in args.datasets:
        roots = {'gt': './Dataset/{}/gt/'.format(dataset),
                 'pred': args.pred_root + 'pred_' + num + '/{}/'.format(dataset)}
